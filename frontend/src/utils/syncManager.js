...
export const syncPendingData = async (onSyncSuccess) => {
  ...
  console.log(`Starting background sync for ${queue.length} pending offline orders...`);
  const failedItems = [];
  ...
  for (const item of queue) {
    try {
      ...
      if (response.ok) {
        ...
        console.log(`Successfully synced offline order. Assigned Bill: ${serverData.order?.bill_number}`);
